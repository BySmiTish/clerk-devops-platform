...
async function setReaction(reaction) {
  ...
  try {
    ...
    if (r.status === 429) {
      ...
      showToast(`Too many requests. Try again in ${sec} seconds.`, Math.min(4000, sec * 1000));
      throw new Error(`Too many requests. Try again in ${sec} seconds.`);
    }
...
async function loadComments(opts = {}) {
  ...
  try {
    ...
    $("commentsCount").textContent = `(${roots.length})`;
...
async function postReply(parentCommentId, author, body) {
  ...
  if (r.status === 429) {
    const sec = data.retry_after || 60;
    showToast(`Too many requests. Try again in ${sec} seconds.`);
    throw new Error(`Too many requests. Try again in ${sec} seconds.`);
  }
  if (!r.ok) throw new Error(data.detail || `HTTP ${r.status}`);
  return data;
...
if (commentsListEl) commentsListEl.addEventListener("click", async (e) => {
  ...
  if (action === "reply") {
    ...
    try {
      const data = await postReply(rootId, authorVal, body);
